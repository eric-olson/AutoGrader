...
# build tests
CPPARGS = ['g++', '-g', '-rdynamic', '-o', 'tests', '-I.', '-I/usr/local/include/StanfordCPPLib', '-std=c++11', '-pthread']
CPPARGS = CPPARGS + ['tests.cpp', 'main.o', 'sighandler.o', '/usr/local/lib/gtest/libgtest.a', '/usr/local/lib/StanfordCPPLib/libStanfordCPPLib.a']
...
def runtests(dir):
    ...
    try:
        ...
        print('<h2 style="color:red"><b>Execution timed out: infinite loop?</b></h2>')
        error_printed = True        
    ...
    if "Received signal" in exec_output:
        print("<h2 style='color:red'><b>There were errors executing the tests.</b></h2>")
    ...
    elif os.path.exists("backtrace.txt"):
    ...
    elif not error_printed:
        print("<h2 style='color:red'>Hm, the tests failed early.  It is probable you have a memory exception error (segmentation fault).</h2>")
